...
import static com.example.task.common.exception.code.ErrorCode.INVALID_REQUEST;
import static com.example.task.common.exception.code.ErrorCode.METHOD_NOT_ALLOWED;
...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ApiResponse<?>> handleBusinessException(BaseException e) {
        ...
        return ResponseEntity
                ...
                .body(ApiResponse.failWithErrorCode(e.getErrorCode().getStatus(), e.getMessage(), e.getErrorCode().getCode()).getBody());
    }

    //Admin 권한이 없을 경우
    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<ApiResponse<?>> handleAccessDeniedException(AccessDeniedException e) {
        log.error("Error Code: {}, Message: {}", ErrorCode.ACCESS_DENIED.getCode(), e.getMessage());

        return ResponseEntity
                .status(ErrorCode.ACCESS_DENIED.getStatus())
                .body(ApiResponse.failWithErrorCode(ErrorCode.ACCESS_DENIED.getStatus(), e.getMessage(), ErrorCode.ACCESS_DENIED.getCode()).getBody());
    }
